...
def validate_vcf(vcf: Union[TextIO, str],
                 out_dir: str = None,
                 fasta: Fasta = None):
    """
    Reads a VCF, validates it and fixes chromosome names if necessary.
    By default, with `Fasta` = `None`, 'chr' string will be added
    to the VCF, in case it doesn't exist.

    :param Union[TextIO, str] vcf: VCF file
    :param str out_dir: Output directory to
    write the validated VCF file. Default: `None`,
    where same directory of `vcf` is used.
    :param Fasta fasta: Fasta pyfaidx object to check
    chromosome names. Default: `None`-
    :return str: Path to the properly formatted VCF file
    """
    ...
    vcf_object = VCF(vcf)
    all_chromosomes_vcf = vcf_object.seqnames
    primary_vcf = [chrom for chrom in all_chromosomes_vcf if chrom in PRIMARY_CHROMOSOMES]
    out = os.path.dirname(vcf) if out_dir is None else out_dir

    if fasta is None:
        # If VCF without 'chr', string will be added by default
        if len([chrom for chrom in primary_vcf if chrom.startswith("chr")]) == 0:
            logger.info("VCF file has not 'chr' string on its chromosome names. "
                         "Since a Fasta object wasn't provided to ensure correct "
                         "chromosome naming, 'chr' string will be added in the "
                         "VCF primary chromosomes by default.")

            return _fix_chr(vcf_object, out)

        else:
            return vcf

    else:
        # get Fasta keys that belong to primary chrom
        primary_fasta = [k for k in fasta.keys() if k in PRIMARY_CHROMOSOMES]

        # if all vcf in fasta, that's fine
        if all(vcf_chrom in primary_fasta for vcf_chrom in primary_vcf):
            return vcf

        else:
            # if Fasta has 'chr' but the vcf not,
            # let's add those
            if all(c.startswith('chr') for c in primary_fasta):
                logger.info("Fasta header contain 'chr' in the primary "
                             "chromosomes. We will add that in the VCF "
                             "as well.")
                return _fix_chr(vcf_object, out)

            # if Fasta doesn't have 'chr', but VCF does have
            elif all(not c.startswith('chr') for c in primary_fasta):
                logger.info("VCF file has 'chr' string in the primary "
                             "chromosomes, while Fasta used doesn't. We "
                             "will remove it from the VCF to force the match"
                             "in the naming.")
                return _fix_chr(vcf_object, out, add=False)
